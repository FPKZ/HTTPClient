import { CollectionTemplate, CollectionItem } from "./postman-translator";
...
export class AxiosFormatter {
  format(internalModel: CollectionTemplate): Record<string, any> {
    const result: Record<string, any> = {};

    const processRoutes = (routes: CollectionItem[], parentPath = "") => {
  ...
  private _formatRequest(item: CollectionItem): any {
    return {
...
export class HttpFormatter {
  format(internalModel: CollectionTemplate | any): string {
    ...
    if (
      (internalModel as any).environments &&
      Array.isArray((internalModel as any).environments)
    ) {
      const activeEnvs = (internalModel as any).environments.filter(
        (env: any) => env.enabled,
      );
      if (activeEnvs.length > 0) {
        activeEnvs.forEach((env: any) => {
  ...
  private _processRoutes(routes: CollectionItem[]): string {
    let content = "";
    routes.forEach((route) => {
      if (route.items && Array.isArray(route.items)) {
  ...
  private _formatRequest(route: CollectionItem): string {
    ...
    if (req.body && req.body.mode !== "none") {
      ...
      if (req.body.mode === "inputs" || req.body.mode === "formdata") {
        if (req.body.mode === "formdata") {
          ...
          req.body.content.forEach((i: any) => {
  ...
  private _getMethodComment(method: string): string {
    switch (method) {
      case "GET":
  ...
  private _formatFormData(formData: any[]): string {
