...
export default function ExportModal() {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const collectAllRouteIds = (items: any[]) => {
        items.forEach((item) => {
          if (item.type === "route") {
  ...
  const toggleRouteSelection = (itemId: string, item: any) => {
  ...
  const toggleAllEnvs = () => {
    if (selectedEnvIds.size > 0) {
      ...
      setSelectedEnvIds(new Set(collection.environments.map((e: any) => e.id)));
    }
  };
  ...
  const getItemCheckState = (item: any) => {
    if (item.type === "route") {
  ...
  const handleExport = async () => {
    ...
    if (exportFormat === "json") {
      (window as any).electronAPI.saveFile({ content: exportData });
    } else if (exportFormat === "http") {
      (window as any).electronAPI.exportHttp({ content: exportData });
    }
      ...
      <div key={item.id}>
        ...
        >
          ...
          {item.type === "folder" && (
            <button
              ...
              className="p-0.5 hover:bg-zinc-700 rounded transition-colors border-none bg-transparent cursor-pointer"
            >
              {isExpanded ? (
        ...
        {item.type === "folder" && isExpanded && item.items && (
          <div className="flex flex-col">
            {item.items.map((child: any) => renderTreeItem(child, depth + 1))}
          </div>
        )}
      </div>
    );
        ...
        >
              ...
              <div className="px-4 py-3 border-b border-zinc-800! bg-zinc-900/30">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    className="text-xs! text-yellow-500 hover:text-yellow-400 font-medium border-none bg-transparent cursor-pointer"
                  >
                ...
                <p className="text-xs! text-zinc-500 m-0 text-left">
                  {selectedRouteIds.size === 0
                    ? "Nenhuma"
                    : selectedRouteIds.size}{" "}
                  {selectedRouteIds.size === 1 ? "rota" : "rotas"} selecionada
                  {selectedRouteIds.size === 1 ? "" : "s"}
                </p>
              </div>

              <div className="flex-1 overflow-y-auto p-2">
                {collection.items.map((item: any) => renderTreeItem(item))}
              </div>
            </div>
            ...
            <div className="w-fit flex flex-col min-w-[45%] max-w-[50%]">
              <div className="px-4 py-3 border-b border-zinc-800! bg-zinc-900/30">
                ...
                <p className="text-xs! text-zinc-500 m-0 text-left">
                  ...
                  {selectedEnvIds.size === 1 ? "ambiente" : "ambientes"}{" "}
                  selecionado{selectedEnvIds.size === 1 ? "" : "s"}
                </p>
              </div>

              <div className="flex-1 overflow-y-auto p-2">
                {collection.environments.map((env: any) => (
                  <div
                    key={env.id}
                  ...
                  >
                    ...
                    <span className="text-sm text-zinc-200 flex-1 truncate text-left">
                      {env.name}
                    </span>
          ...
          <div className="px-4 py-3 border-t border-zinc-800! bg-zinc-900/50 flex justify-between items-center">
            ...
            <div className="flex gap-3">
              <Dialog.Close asChild>
                <button className="h-10 px-4 rounded-lg! bg-zinc-800 text-zinc-300 text-sm! font-bold hover:bg-zinc-700 transition-colors border-none cursor-pointer">
                  Cancelar
                </button>
              </Dialog.Close>
              <button
                ...
                className="h-10 px-4 rounded-lg! bg-yellow-600 text-zinc-950 text-sm! font-bold hover:bg-yellow-500 transition-all shadow-lg shadow-yellow-600/10! disabled:opacity-50 disabled:cursor-not-allowed border-none cursor-pointer"
              >
                Exportar
              </button>
            </div>
