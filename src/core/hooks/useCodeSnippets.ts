...
interface UseCodeSnippetsProps {
  request: any;
}

export function useCodeSnippets({ request }: UseCodeSnippetsProps) {
  const [activeCategory, setActiveCategory] = useState("all");
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedSnippet, setSelectedSnippet] = useState<SnippetMetadata | null>(null);
  ...
  const snippetsMetadata = useMemo(() => {
    const list: SnippetMetadata[] = [];
    let idCounter = 1;

    (supportedLanguages as any[]).forEach((lang) => {
      lang.variants.forEach((variant: any) => {
        list.push({
          id: idCounter++,
  ...
  const categories = useMemo(() => {
    const availableCategories = (supportedLanguages as any[])
