...
export function useNewCollection() {
  ...
  const createTestRoute = (method: string) => ({
  ...
  const newCollection = (name = "Nova Coleção", desc = "", routes: string[] = []) => {
    if (!name) {
      name = "Nova Coleção";
    }
    if (!desc) {
  ...
  const triggerNewCollection = useCallback(async () => {
    try {
      ...
      // Passamos false ou true baseado na lógica necessária, mas useHistory.ts espera boolean
      await handleSaveCollection(true);
    } catch (error) {
